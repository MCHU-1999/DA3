...
from depth_anything_3.utils.RTS import rotate_translate_scale
...
from depth_anything_3.utils.normal_utils import render_normals
from depth_anything_3.utils.depth_utils import update_depths
...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def inference(
        ...
        # Scale, Rotate and Translate the final Prediction
        if gnd_mask_paths is not None and bldg_mask_paths is not None:
            prediction = rotate_translate_scale(
                prediction,
        ...
        # Add normal maps after we aligned predicted extrinsics to our reference extrinsics
        prediction = self._add_normals(prediction)
    ...
    def _update_depths(self, prediction: Prediction, conf_thresh_percentile=None) -> Prediction:
        """Re-render depth maps with masks masks"""
        depths = update_depths(
            prediction.depth,
            prediction.conf,
            conf_thresh_percentile
        )
        prediction.depth = depths
        return prediction
    
    def _add_normals(self, prediction: Prediction, conf_thresh_percentile=None) -> Prediction:
        """Add normal maps to prediction (from depth maps)."""
        normals = render_normals(
            prediction.depth,
            prediction.extrinsics,
            prediction.intrinsics
        )
        prediction.normal = normals
        return prediction
