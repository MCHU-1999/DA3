...
def _depths_to_world_points_with_mask(
    depth: np.ndarray,
    K: np.ndarray,
    ext_w2c: np.ndarray,
    mask: np.ndarray,
    ...
    N, H, W = depth.shape
    if mask.shape != (N, H, W):
        raise ValueError(f"mask shape must be {(N, H, W)}, got {mask.shape}")
    ...
    for i in range(N):
        d = depth[i]  # (H,W)
        valid = np.isfinite(d) & (d > 0) & mask[i].astype(bool)
        if conf is not None:
            valid &= conf[i] >= conf_thr
        if not np.any(valid):
            continue
