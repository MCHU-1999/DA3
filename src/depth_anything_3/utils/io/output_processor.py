...
class OutputProcessor:
    ...
    def __call__(self, model_output: dict[str, torch.Tensor]) -> Prediction:
        ...
        # normal = self._extract_normal(model_output)
    ...
    def _extract_normal(self, model_output: dict[str, torch.Tensor]) -> np.ndarray:
        """
        Extract depth tensor from model output and convert to normal (in numpy).

        Args:
            model_output: Model output dictionary

        Returns:
            Normal array with shape (N, H, W, 3)
        """
        depth = model_output["depth"].squeeze(0).squeeze(-1)  # (N, H, W)
        ...
        normals = render_normals(depth, extrinsics, intrinsics)
